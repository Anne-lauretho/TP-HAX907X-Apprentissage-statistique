...
Il est important de noter que l’accuracy de 70 % pour $C = 10^{-5}$ ne reflète pas une véritable performance du modèle. 
Ce résultat est dû au déséquilibre des classes : comme il y a davantage d’images de Colin Powell, le classificateur prédit systématiquement cette classe et atteint ainsi artificiellement 70 % de bonnes réponses. 
Cette valeur élevée d’accuracy n’indique donc pas une capacité réelle de reconnaissance, mais seulement un biais lié à la distribution des données.
Cela souligne que l’accuracy seule ne reflète pas la véritable performance de classification. 
Les métriques de *précision (precision)*, de *rappel (recall)* et de *F1 (f1-score)* permettent d’évaluer la capacité réelle du modèle à reconnaître chaque catégorie : pour $C = 10^{5}$, la précision et le rappel s’améliorent pour les deux classes (Donald Rumsfeld : precision = 0.82, recall = 0.76 ; Colin Powell : precision = 0.90, recall = 0.93), ce qui montre une amélioration effective de la qualité des prédictions.
...
Pour $C = 10^{-5}$, l'image du coefficient est plus floue et les points rouges (importants) et bleus (sans importance) de l'image sont plus grands, c'est-à-dire plus généraux. Les caractéristiques les plus importantes sont prises en compte en premier, telles que la taille du front et la ligne des cheveux, la forme du nez et des lèvres, le cou et l'arrière-plan (cela peut être dû au fait que les photos ont été prises sur un arrière-plan récurrent dans l'échantillon).

Pour $C = 10^{5}$, le modèle devient beaucoup plus sensible et fragmente toutes les parties de l'image. On constate ainsi que les points rouges et bleus sont devenus beaucoup plus petits.
...
Les résultats nous montrent que sans variables de nuisance, le SVM obtient une précision d'environ $0.92$ sur l'échantillon de test. En revanche, après l'ajout de $300$ variables aléatoires, la précision chute à environ $0.55$, soit proche du niveau du hasard. 
...
Le graphique obtenu montre que, pour un faible nombre de variables de nuisance, l’accuracy reste relativement élevée, ce qui confirme que le modèle continue à exploiter efficacement les caractéristiques pertinentes.
Lorsque le nombre de variables de nuisance augmente, on observe des fluctuations de l’accuracy sur l’échantillon de test, dues à la nature aléatoire du bruit ajouté et au choix du paramètre optimal C à chaque itération.
Il est important de noter que la comparaison « sans bruit » et « avec 300 variables de nuisance » montre effectivement une dégradation des performances du modèle.

Cependant, la tendance générale du graphique n’indique pas que l’augmentation du nombre de variables de nuisance jusqu’à 1000 entraînera une baisse supplémentaire de l’accuracy : pour un grand nombre de variables de bruit indépendantes, le SVM linéaire commence à en lisser l’influence et la performance se stabilise à un niveau proche de celui d’origine.


### Question 6 - PCA

Pour suivre l’influence du paramètre C sur la précision des résultats, nous avons utilisé le code
...
Le code présenté permet de générer un graphique illustrant l’influence du nombre de composantes PCA sur la précision du modèle ainsi que sur le temps nécessaire pour traiter la requête. Sur le graphique, l’axe X représente le nombre de composantes PCA, l’axe Y gauche indique la précision (courbe bleue), et l’axe Y droit montre le temps de calcul en secondes (courbe rouge, échelle logarithmique).  

Il est important de noter que l’exécution complète du code prend un temps considérable, car pour chaque nombre de composantes PCA, les opérations suivantes sont effectuées :  
...
L’utilisation du paramètre `svd_solver='randomized'` permet d’accélérer le calcul de la PCA sur de grands ensembles de données tout en conservant une approximation précise des composantes principales.  

En raison de ces étapes, le calcul pour toutes les composantes de 20 à 200 (par pas de 10) peut durer plusieurs heures.  

Pour les besoins du rapport, nous présentons donc **les résultats déjà générés**, afin d’illustrer la relation entre la précision et le temps de calcul en fonction du nombre de composantes PCA. Néanmoins, le code est entièrement fonctionnel et peut être exécuté de manière autonome en utilisant la version disponible dans le dépôt GitHub. (PUT THIS IS README MAYBE?) 

<img src="images/output_q6_1.png" alt="first try" style="display:inline-block; width:48%; margin-right:2%;">
<img src="images/output_q6_2.png" alt="second try" style="display:inline-block; width:48%;">

Les graphiques montrent l’influence du nombre de composantes PCA sur la précision du modèle et sur le temps de traitement. Comme nous avons utilisé la fonction `sklearn.decomposition.PCA(svd_solver='randomized')`, la répartition des classes diffère légèrement à chaque exécution en raison de l’initialisation aléatoire, ce qui rend la qualité de la classification instable.  

Cependant, on peut constater que le temps nécessaire au traitement des requêtes reste à peu près constant. Il atteint son pic entre 32 et 52 composantes, ce qui s’explique par le fait que, pour un petit nombre de composantes, le PCA tente de conserver le maximum de variance des caractéristiques originales. À ce stade, la charge de calcul pour le SVM est plus élevée, car les données contiennent encore une quantité significative d’information utile pour la construction de l’hyperplan, et l’algorithme passe plus de temps à optimiser. Ensuite, lorsque le nombre de composantes augmente, le PCA commence à disperser les caractéristiques significatives, réduisant ainsi la charge sur le SVM et entraînant une diminution du temps de traitement.  

<img src="images/output_q6_0_old.png" style="display:inline-block">
...
Dans le prétraitement initial des données, la normalisation était appliquée **avant la séparation** des échantillons d’entraînement et de test. Cela signifie que la moyenne et l’écart-type ont été calculés sur l’ensemble du jeu de données, ce qui a entraîné une **fuite d’information** depuis l’échantillon de test vers l’entraînement. Ce biais systématique (biais de prétraitement) peut donner une estimation trop optimiste de la précision du modèle.  

Dans la version corrigée, nous avons d’abord séparé les données en ensembles d’entraînement et de test, puis nous avons normalisé **uniquement à partir de l’ensemble d’entraînement**. Les valeurs de moyenne et d’écart-type ainsi calculées sont ensuite utilisées pour normaliser l’échantillon de test. Cette approche élimine le biais et permet d’évaluer la performance réelle du modèle sur des données non vues.  
...
Après cela, exécutons le même code pour suivre la différence de sortie.
...
Après correction du prétraitement et normalisation uniquement sur l’ensemble d’entraînement, la meilleure valeur du paramètre C a fortement diminué, passant de $C = 10^{-3}$ à $C = 10^{-5}$. Cela s’explique par le fait que le modèle ne bénéficie plus d’informations provenant de l’ensemble de test lors du scaling des caractéristiques. 

Ainsi, ce changement reflète une évaluation plus réaliste de la capacité de généralisation du modèle et non une dégradation de celui-ci.
...
- pour $C = 10^{-5}$, l’accuracy est passée de $0.70$ à $0.68$. Le modèle ne reconnaît toujours pas la classe *Donald Rumsfeld*, tandis que la précision et le rappel pour *Colin Powell* restent élevés.

- pour $C = 10^{-}$, a baisse est plus marquée : l’accuracy chute de $0.88$ à $0.50$. Le modèle distingue moins bien les deux classes : précision et rappel pour *Donald Rumsfeld* et *Colin Powell* sont fortement réduits.

La raison de cette variation réside dans la correction du biais de prétraitement : la normalisation est désormais effectuée uniquement sur l’ensemble d’entraînement, et le modèle n’utilise plus d’informations provenant de l’ensemble de test pour le scaling des caractéristiques. Cela rend l’évaluation sur l’ensemble de test plus réaliste, mais fait disparaître les performances artificiellement élevées observées auparavant.
...
Les résultats nous montrent que sans variables de nuisance, le SVM obtient une précision d'environ $0.89$ sur l'échantillon de test. En revanche, après l'ajout de $300$ variables aléatoires, la précision chute à environ $0.53$, soit proche du niveau du hasard. 

On peut constater que les performances sont légèrement inférieures par rapport aux résultats initiaux de la question 5. Cela s’explique par le fait que nous utilisons désormais un ordre correct des opérations : séparation des ensembles d’entraînement et de test avant normalisation. En conséquence, l’évaluation sur l’ensemble de test est plus réaliste, et le modèle n’utilise plus d’informations provenant des données de test lors de la normalisation, ce qui entraîne une diminution de la précision lorsque des variables de nuisance sont ajoutées.

<img src="images/output_q6_apres_q7.png" style="display:inline-block">

Après avoir modifié l’ordre des opérations sur l’échantillon et corrigé l’erreur de fuite d’information, lors de l’utilisation du code pur le question 6, on constate que le temps total d’exécution du code a considérablement diminué. Cela s’explique par le fait que la normalisation est désormais effectuée uniquement sur l’ensemble d’entraînement, et que le modèle ne réalise plus de calculs inutiles sur l’ensemble complet des données.
Cependant, la tendance générale du temps de traitement reste la même : on observe un pic entre 32 et 52 composantes PCA, puis le temps diminue progressivement.
